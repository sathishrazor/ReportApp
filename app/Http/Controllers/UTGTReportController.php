...
class UTGTReportController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<div class="btn btn-group"><a href="/utgtreport/'.$row->id.'/edit" class="edit btn btn-primary btn-sm editItem">Edit</a>';
    ...
    {
        $record = UTGTReport::findOrFail($id);
        return  view('utgtreports.edit',["record"=>$record]);
    }

    public function update($id,Request $request)
    {
        $rec = UTGTReport::findOrFail($id);
